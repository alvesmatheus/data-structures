...
import abstractions.HashFunction;
...
public class HashFunctionQuadraticProbing<T> implements ProbingHashFunction<T> {
	 ...
	 * 
	 * @param tableSize the size of the table that will contain the hash function.
	 * @param method    The hashing method that must be used by the linear probing.
	 * @param c1        The first constant used on the hash calculation.
	 * @param c2        The second constant used on the hash calculation.
	 * 
	 */
	@SuppressWarnings("unchecked")
	public HashFunctionQuadraticProbing(int tableSize, ClosedAddressMethod method) {
	 ...
	 * 
	 * @param method    The method that must be used by the hash function.
	...
	public int hash(T element, int probe) {
		...
		hashKey = basicHashFunction.hash(element);
		hashKey = ((hashKey + c1 * probe + c2 * (probe * probe)) % this.tableSize);
