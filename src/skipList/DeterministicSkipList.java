...
import abstractStructures.SkipList;
...
public class DeterministicSkipList<T> implements SkipList<T> {
	 ...
	 * 
	 * @param maxHeight
	 *            The maximum height allowed to the skip list nodes.
	 * 
	 */
	 ...
	 * @param height
	 *            The height of the new node that will be created.
	 * 
	 */
	 ...
	 * 
	 * @param key
	 *            the key of the node being searched for.
	 * 
	...
	/**
	 * Creates an array containing all the keys of the skip list ordered by the
	 * heights of the nodes containing these keys (from the highest ones to the
	 * shortest ones).
	 * 
	 * @return an array of the skip list keys ordered by node heights.
	 * 
	 */
	public int[] keysOrderedByHeight() {
		int position = 0;
		int[] array = new int[this.size()];

		for (int i = (this.height() - 1); i >= 0; i--) {
			SkipListNode<T> node = this.getHead();

			while (!node.getForward(i).equals(this.getTail())) {
